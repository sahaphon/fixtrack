    ...
    const loadData = async (type, search, offset, limit) => {
        ...
        if (res != undefined) {
            const data = res.result.map((value, index) => {
                return {
                    ...value,
                    key: value.machine + value.data_date,
                    performance:
                        numeral((value.qty * 100) / (value.hours * 72)).format('0.00') + '%',
                    quality:
                        numeral(((value.qty - value.waste_qty) * 100) / value.qty).format('0.00') +
                        '%',
                    availability:
                        numeral(
                            (value.run_time * 100) /
                                (value.run_time + value.lost_time + value.down_time),
                        ).format('0.00') + '%',
                    oee:
                        numeral(
                            (value.qty / (value.hours * 72)) *
